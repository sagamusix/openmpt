...
BOOL CViewPlugins::PreTranslateMessage(MSG *pMsg)
{
	if(pMsg)
	{
			...
			(pMsg->message == WM_SYSKEYDOWN) || (pMsg->message == WM_KEYDOWN))
		{
			...
			if (ih->KeyEvent(kCtxViewPlugins, nChar, nRepCnt, nFlags, ih->GetKeyEventType(nFlags)) != kcNull)
			{
...
void CViewPlugins::UpdateScrollSize()
{
	...
	m_linePen.CreatePen(PS_SOLID, Util::ScalePixels(1, m_hWnd), RGB(40, 40, 40));  // TODO
	m_activeLinePen.DeleteObject();
	m_activeLinePen.CreatePen(PS_SOLID, Util::ScalePixels(2, m_hWnd), RGB(40, 40, 40));  // TODO
...
{
	if(m_selectedNode != nullptr && m_selectedNode->Rect().PtInRect(point))
	{
...
void CViewPlugins::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if(nFlags & MK_LBUTTON)
	{
		if(m_selectedNode != nullptr)
		{
			CPoint newPos = m_selectedNode->Position() + (point - m_mouseDownPoint);
			m_selectedNode->Position(newPos);
			if(auto pluginNode = std::dynamic_pointer_cast<PluginNode>(m_selectedNode); pluginNode != nullptr)
			{
				CSoundFile &sndFile = GetDocument()->GetSoundFile();
...
void CViewPlugins::OnPaint()
{
	...
	for(auto &node : m_nodes)
	{
		if(node != m_selectedNode && (dirty.Intersects(node->ScreenRect()) || dirty.Intersects(node->ScreenRectLines())))
		{
	...
	if(m_selectedNode != nullptr)
	{
		if(dirty.Intersects(m_selectedNode->ScreenRect()) || dirty.Intersects(m_selectedNode->ScreenRectLines()))
		{
			m_selectedNode->Render(m_offScreenDC);
		}
	}
