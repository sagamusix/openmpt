...
namespace
{
	...
	auto sign(mix_t v) { return static_cast<mix_t>(sgn(v)); }
...
{
	return IsInRange(m_sampleFrequency, MIN_FREQ, MAX_FREQ)
		&& IsInRange(m_sampleLength, MIN_LENGTH, MAX_LENGTH);
}
...
bool SampleGenerator::RenderSample(CSoundFile &sndFile, SAMPLEINDEX smp)
{
	...
	if(sample.AllocateSample())
	{
		...
		ctrlSmp::ReplaceSample(sample, pSample, length, sndFile);
...
BEGIN_MESSAGE_MAP(SmpGenDialog, CDialog)
...
SmpGenDialog::SmpGenDialog(uint32 freq, SmpLength len, SampleGenerator::ClipMethod clipping, const CString &expr, CWnd *parent)
    : CDialog(IDD_SAMPLE_GENERATOR, parent)
    , m_sampleLength(len)
...
BOOL SmpGenDialog::OnInitDialog()
{
	...
	::SendMessage(*GetDlgItem(IDC_BUTTON_SHOW_EXPRESSIONS), WM_SETFONT, reinterpret_cast<WPARAM>(m_hButtonFont.m_hObject), MAKELPARAM(TRUE, 0));
...
BEGIN_MESSAGE_MAP(SmpGenPresetDlg, CDialog)
...
SmpGenPresetDlg::SmpGenPresetDlg(SmpGenPresets &presets, CWnd *parent)
	: CDialog(IDD_SAMPLE_GENERATOR_PRESETS, parent)
	, m_presets(presets)
{
}
