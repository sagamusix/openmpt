...
CString SampleGenerator::m_expression = _T("return math.sin(xp * math.pi)");
...
bool SampleGenerator::TestExpression()
{
	try
	{
		lua->script("function sampleGenerator(x, xp, len, lens, freq) " + mpt::ToCharset(mpt::Charset::UTF8, m_expression) + " end");
		...
		if(res.get_type() != sol::type::number)
			throw sol::error{"Function does not return a number"};
...
BEGIN_MESSAGE_MAP(SmpGenDialog, ResizableDialog)
...
SmpGenDialog::SmpGenDialog(uint32 freq, SmpLength len, SampleGenerator::ClipMethod clipping, const CString &expr, CWnd *parent)
	: ResizableDialog{IDD_SAMPLE_GENERATOR, parent}
	, m_sampleLength{len}
...
{
	m_presets.assign({
		{_T("A440"), _T("return math.sin(xp * math.pi / 50 * 440 * len / freq)")},
		{_T("Bass Drum"), _T("return math.sin(math.sqrt(xp) * math.pi * 10) * math.exp(math.min(0, 15 - xp / 5)) + (math.random() * 2 - 1) * math.exp(-xp)")},
		{_T("Brown Noise (kind of)"), _T("return (math.random() - 0.5) * 0.1 + smp(x - 1) * 0.9")},
		{_T("Noisy Saw"), _T("return (x % 800) / 800.0 - 0.5 + math.random() * 0.1")},
		{_T("PWM Filter"), _T("return pwm(x, 50 + math.sin(xp * math.pi / 100) * 40, 100) + tri(x, 50)")},
		{_T("Fat PWM Pad"), _T("return pwm(x, xp, 500) + pwm(x, math.abs(50 - xp), 1000)")},
		{_T("Dual Square"), _T("return iif((x % 100) < 50, x % 200, -(x % 200) + 50)")},
		{_T("Noise Hit"), _T("return math.exp(-xp) * (math.random() * x - x / 2)")},
		{_T("Laser"), _T("return math.sin(xp * math.pi * 100 / xp ^ 2) * 100 / math.sqrt(xp)")},
		{_T("Noisy Laser Hit"), _T("return (math.sin(math.sqrt(xp) * 100) + math.random() - 0.5) * math.exp(-xp / 10)")},
		{_T("Twinkle, Twinkle..."), _T("return math.sin(x * math.pi * 100 / xp) * 100 / math.sqrt(xp)")},
		{_T("FM Tom"), _T("return math.sin(xp * math.pi * 2 + (xp / 5 - 50) ^ 2) * math.exp(-xp / 10)")},
		{_T("FM Warp"), _T("return math.sin(math.pi * xp / 2 * (1 + (1 + math.sin(math.pi * xp / 4 * 50)) / 4)) * math.exp(-(xp / 8) * 0.6)")},
		{_T("Super Pulse"), _T("return pwm(x * 0.99, 50, 100) + pwm(x * 1.01, 50, 100) + pwm(x * 0.985, 50, 100) + pwm(x * 1.015, 50, 100)")},
		{_T("Flanging Saw"), _T("return x % 100 + (x * 0.99) % 100 + (x * 1.01) % 100 - 150")},
		//{_T("Weird Noise"), _T("return math.random() * 0.1 + smp(x - math.random() * xp) * 0.9")},
	});
}
