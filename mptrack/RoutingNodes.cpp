...
{
	return m_owner.m_selectedNode.get() == this;
}
...
{
#define MIXCOLOR(factor) RGB(mpt::saturate_cast<uint8>(factor * r), mpt::saturate_cast<uint8>(factor * g), mpt::saturate_cast<uint8>(factor * b))
...
void PluginNode::Render(CDC &dc)
{
	...
	if(plugin != nullptr)
	{
		...
		for(size_t i = 0; i < m_vuOutputs.size(); i++)
		{
			if(numSamples)
			{
				m_vuMeters[i].SetDecaySpeedDecibelPerSecond(TrackerSettings::Instance().VuMeterDecaySpeedDecibelPerSecond);
				m_vuMeters[i].Process(&m_vuOutputs[i], 1, 1);
			}
			VUMeterUI vu;
