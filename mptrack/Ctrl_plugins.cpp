...
{
	// Initial Update
	if (!m_bInitialized) UpdateView(UpdateHint().ModType());
...
void CCtrlPlugins::UpdateView(UpdateHint hint, CObject *pObj)
{
	...
	if(plugHint.GetType()[HINT_MODTYPE | HINT_PLUGINNAMES] && updateThisPlug)
	{
	...
	{
		m_CbnPlugin.Update(m_sndFile, PluginChannel(m_nCurrentPlugin + 1), PluginComboBox::ShowEmptySlots | PluginComboBox::ShowLibraryNames);
...
{
	PLUGINDEX nPlugin = m_CbnPlugin.GetSelectionPlugin();
	if(nPlugin != PLUGINDEX_INVALID)
	{
		m_nCurrentPlugin = nPlugin - 1;
		UpdateView(PluginHint(m_nCurrentPlugin + 1).Info().Names());
		SendViewMessage(VIEWMSG_SETCURRENTPLUGIN, m_nCurrentPlugin);
	}
	m_CbnPreset.SetCurSel(0);
...
{
	if(m_nCurrentPlugin >= MAX_MIXPLUGINS) return;
	m_sndFile.m_MixPlugins[m_nCurrentPlugin].SetBypass(IsDlgButtonChecked(IDC_CHECK10) != BST_UNCHECKED);
	SetModified();
}
...
{
	if(m_nCurrentPlugin >= MAX_MIXPLUGINS) return;
	m_sndFile.m_MixPlugins[m_nCurrentPlugin].SetMixMode((uint8)m_CbnSpecialMixProcessing.GetCurSel());
	SetModified();
}
...
{
	if(m_nCurrentPlugin >= MAX_MIXPLUGINS) return;
	m_sndFile.m_MixPlugins[m_nCurrentPlugin].SetWetMix(IsDlgButtonChecked(IDC_CHECK11) != BST_UNCHECKED);
	SetModified();
}
...
{
	if(m_nCurrentPlugin >= MAX_MIXPLUGINS) return;
	m_modDoc.TogglePluginEditor(m_nCurrentPlugin, CMainFrame::GetInputHandler()->ShiftPressed());
	return;
}
...
void CCtrlPlugins::OnMovePlugToSlot()
{
	...
	if(dlg.DoModal() == IDOK)
	{
		...
		m_CbnPlugin.SetSelection(dlg.GetSlot() + 1);
		OnPluginChanged();
...
bool CCtrlPlugins::MovePlug(PLUGINDEX src, PLUGINDEX dest, bool bAdjustPat)
{
	...
	m_sndFile.m_MixPlugins[src] = SNDMIXPLUGIN();
...
void CCtrlPlugins::OnInsertSlot()
{
	...
	if(Reporting::Confirm(prompt) == cnfYes)
	{
		...
		m_CbnPlugin.SetSelection(m_nCurrentPlugin + 1);
		OnPluginChanged();
		m_modDoc.UpdateAllViews(nullptr, PluginHint().Names(), this);
