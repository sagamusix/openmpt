...
struct Sample : public Obj
{
	...
	bool Resample(int newRate, EnumInterpolationType::values interpolationType, sol::optional<bool> updateOffsets, sol::optional<SmpLength> start, sol::optional<SmpLength> end)
	{
		auto sndFile = SndFile();
		auto sample = GetSample();
		ThrowIfOPL(sample);
		...
		if(!SampleEdit::Resample(sample, start.value_or(0u), end.value_or(sample->nLength), mpt::saturate_cast<uint32>(newRate), mode, sndFile, updateOffsets.value_or(false), prepareSampleUndoFunc, preparePatternUndoFunc))
			return false;
