...
struct SampleLoop : public Obj
{
	...
	static void Register(sol::table &table)
	{
		table.new_usertype<SampleLoop>("SampleLoop"
			...
			, "crossfade", &SampleLoop::Crossfade
			);
	}
	...
	{
		auto typeIndex = EnumLoopType::Parse(type);
		if(typeIndex == EnumLoopType::Enum::INVALID_VALUE)
			throw "Invalid loop type (must be one of openmpt.Song.loopType)";
		...
		if(isSustainLoop)
		{
			flags.set(CHN_SUSTAINLOOP, typeIndex != EnumLoopType::Enum::OFF);
			flags.set(CHN_PINGPONGSUSTAIN, typeIndex == EnumLoopType::Enum::PINGPONG && !(sndFile->GetType() & (MOD_TYPE_MOD | MOD_TYPE_S3M)));
		} else
		{
			flags.set(CHN_LOOP, typeIndex != EnumLoopType::Enum::OFF);
			flags.set(CHN_PINGPONGLOOP, typeIndex == EnumLoopType::Enum::PINGPONG && !(sndFile->GetType() & (MOD_TYPE_MOD | MOD_TYPE_S3M)));
		}
		sample->PrecomputeLoops(sndFile, CMainFrame::GetMainFrame()->GetSoundFilePlaying() == sndFile);
	...
	bool Crossfade(double duration, sol::optional<double> fadeLaw, sol::optional<bool> afterLoopFade)
	{
		...
		{
			SampleHint hint = SampleHint(static_cast<SAMPLEINDEX>(m_index)).Data();
			if(sample->uFlags[SMP_KEEPONDISK] && !sample->uFlags[SMP_MODIFIED])
				hint.Names();
			sample->uFlags.set(SMP_MODIFIED);
			SetModified(sample, hint);
			return true;
		}
		return false;
	}
...
struct OPLOperator : public Obj
{
	...
	{
		return EnumOPLWaveform::strings[GetSample()->adlib[8 | opOffset]];
	}
	void SetWaveform(std::string_view waveform)
	{
		const auto waveformIndex = EnumOPLWaveform::Parse(waveform);
		if(waveformIndex == EnumOPLWaveform::Enum::INVALID_VALUE)
			throw "Invalid OPL waveform (must be one of openmpt.Song.OPLWaveform)";
		auto sample = GetSample();
		const uint8 op = 8 | opOffset;
		const uint8 oldValue = sample->adlib[op];
		sample->adlib[op] = std::min(static_cast<uint8>(waveformIndex), (SndFile()->GetType() == MOD_TYPE_S3M) ? uint8(3) : uint8(7));
...
struct SampleCuePoints final : public ProxyContainer<SmpLength>, public Obj
{
	...
	ScopedLock<ModSample> GetSample()
	{
		...
		if(m_index > 0 && m_index <= sndFile->GetNumSamples())
		{
			ModSample &sample = sndFile->GetSample(static_cast<SAMPLEINDEX>(m_index));
			if(sample.uFlags[CHN_ADLIB])
	...
	sol::object Index(size_t position)
	{
		auto sample = GetSample();
		if(position < 1 || position > sample->cues.size())
			throw "Invalid cue point!";
		if(auto cue = sample->cues[position - 1u]; cue < sample->nLength)
			return sol::make_object(m_script, cue);
		else
			return sol::make_object(m_script, sol::nil);
	}

	void NewIndex(size_t position, sol::object value)
	{
		auto sample = GetSample();
		if(position < 1 || position > sample->cues.size())
			throw "Invalid cue point!";
		if(value.is<SmpLength>())
			sample->cues[position - 1u] = value.as<SmpLength>();
		else if(!value.valid())
			sample->cues[position - 1u] = sample->nLength;
		else
			throw "Invalid cue point value (must be integer or nil)";
...
struct Sample : public Obj
{
	...
	static void Register(sol::table &table)
	{
		table.new_usertype<Sample>("Sample"
			...
			, "index", sol::readonly(&Sample::m_index)
			...
			//, "save"
			, "load", & Sample::Load
			, "play", &Sample::PlayNote
			, "stop", &Sample::StopNote
			, "normalize", &Sample::Normalize
			, "remove_dc_offset", &Sample::RemoveDCOffset
			, "amplify", &Sample::Amplify
			, "change_stereo_separation", &Sample::ChangeStereoSeparation
			, "resample", &Sample::Resample
			, "reverse", &Sample::Reverse
			, "silence", &Sample::Silence
			, "invert", &Sample::Invert
			, "unsign", &Sample::Unsign
			, "stretch_hq", &Sample::StretchHQ
			, "stretch_lofi", &Sample::StretchLofi
			//, "autotune"
			, "get_data", &Sample::GetData
			, "set_data", &Sample::SetData
			);
	}
	...
	std::string GetVibratoType() const
	{
		auto sample = GetSample();
	...
	void SetVibratoType(std::string_view type)
	{
		const auto typeIndex = EnumVibratoType::Parse(type);
		if(typeIndex == EnumVibratoType::Enum::INVALID_VALUE)
			throw "Invalid vibrato type (must be one of openmpt.Song.vibratoType)";

		auto sample = GetSample();
		sample->nVibType = static_cast<VibratoType>(typeIndex);
	...
	bool Normalize(sol::optional<SmpLength> start, sol::optional<SmpLength> end)
	{
		auto sample = GetSample();
		ThrowIfOPL(sample);
		if(SampleEdit::NormalizeSample(sample, start.value_or(0u), end.value_or(sample->nLength), SampleChannelSelection::Both, SndFile()))
		{
	...
	double RemoveDCOffset(sol::optional<SmpLength> start, sol::optional<SmpLength> end)
	{
		auto sample = GetSample();
		ThrowIfOPL(sample);
		const double offset = SampleEdit::RemoveDCOffset(sample, start.value_or(0u), end.value_or(sample->nLength), SampleChannelSelection::Both, SndFile());
		if(offset != 0.0)
			SetModified(sample, SampleHint(static_cast<SAMPLEINDEX>(m_index)).Data(), true);
		return offset;
	}

	bool Amplify(double amplifyStart, sol::optional<double> amplifyEnd, sol::optional<SmpLength> start, sol::optional<SmpLength> end, sol::optional<std::string_view> fadeLawAPI)
	{
		Fade::Law fadeLaw = Fade::kLinear;
		if(fadeLawAPI)
		{
			switch(EnumFadeLaw::Parse(*fadeLawAPI))
			{
			case EnumFadeLaw::Enum::LINEAR: fadeLaw = Fade::kLinear; break;
			case EnumFadeLaw::Enum::EXPONENTIAL: fadeLaw = Fade::kPow; break;
			case EnumFadeLaw::Enum::SQUARE_ROOT: fadeLaw = Fade::kSqrt; break;
			case EnumFadeLaw::Enum::LOGARITHMIC: fadeLaw = Fade::kLog; break;
			case EnumFadeLaw::Enum::QUARTER_SINE: fadeLaw = Fade::kQuarterSine; break;
			case EnumFadeLaw::Enum::HALF_SINE: fadeLaw = Fade::kHalfSine; break;
			case EnumFadeLaw::Enum::INVALID_VALUE: throw "Invalid vibrato type (must be one of openmpt.fadeLaw)";
			}
		}

		auto sample = GetSample();
		ThrowIfOPL(sample);
		if(SampleEdit::AmplifySample(sample, start.value_or(0u), end.value_or(sample->nLength), SampleChannelSelection::Both, amplifyStart, amplifyEnd.value_or(amplifyStart), true, fadeLaw, SndFile()))
		{
			SetModified(sample, SampleHint(static_cast<SAMPLEINDEX>(m_index)).Data(), true);
			return true;
		}
	...
	bool Resample(int newRate, std::string_view interpolationType, sol::optional<bool> updateOffsets, sol::optional<bool> updateNotes, sol::optional<SmpLength> start, sol::optional<SmpLength> end)
	{
		auto sndFile = SndFile();
		auto sample = GetSample();
		ThrowIfOPL(sample);
		...
		switch(EnumInterpolationType::Parse(interpolationType))
		{
		case EnumInterpolationType::Enum::DEFAULT:
		case EnumInterpolationType::Enum::R8BRAIN:
			break;
		case EnumInterpolationType::Enum::NEAREST: mode = SRCMODE_NEAREST; break;
		case EnumInterpolationType::Enum::LINEAR: mode = SRCMODE_LINEAR; break;
		case EnumInterpolationType::Enum::CUBIC_SPLINE: mode = SRCMODE_CUBIC; break;
		case EnumInterpolationType::Enum::SINC: mode = SRCMODE_SINC8LP; break;
		case EnumInterpolationType::Enum::INVALID_VALUE: throw "Unknown interpolation type";
		}

		if(!SampleEdit::Resample(sample, start.value_or(0u), end.value_or(sample->nLength), SampleChannelSelection::Both, mpt::saturate_cast<uint32>(newRate), mode, sndFile, updateOffsets.value_or(false), updateNotes.value_or(false), prepareSampleUndoFunc, preparePatternUndoFunc))
			return false;
	...
	bool Reverse(sol::optional<SmpLength> start, sol::optional<SmpLength> end)
	{
		auto sample = GetSample();
		ThrowIfOPL(sample);
		if(SampleEdit::ReverseSample(sample, start.value_or(0u), end.value_or(sample->nLength), SampleChannelSelection::Both, SndFile()))
		{
			SetModified(sample, SampleHint(static_cast<SAMPLEINDEX>(m_index)).Data(), true);
			return true;
		}
	...
	bool Silence(sol::optional<SmpLength> start, sol::optional<SmpLength> end)
	{
		auto sample = GetSample();
		ThrowIfOPL(sample);
		if(SampleEdit::SilenceSample(sample, start.value_or(0u), end.value_or(sample->nLength), SampleChannelSelection::Both, SndFile()))
		{
			SetModified(sample, SampleHint(static_cast<SAMPLEINDEX>(m_index)).Data(), true);
			return true;
		}
	...
	bool Invert(sol::optional<SmpLength> start, sol::optional<SmpLength> end)
	{
		auto sample = GetSample();
		ThrowIfOPL(sample);
		if(SampleEdit::InvertSample(sample, start.value_or(0u), end.value_or(sample->nLength), SampleChannelSelection::Both, SndFile()))
		{
			SetModified(sample, SampleHint(static_cast<SAMPLEINDEX>(m_index)).Data(), true);
			return true;
		}
	...
	bool Unsign(sol::optional<SmpLength> start, sol::optional<SmpLength> end)
	{
		auto sample = GetSample();
		ThrowIfOPL(sample);
		if(SampleEdit::UnsignSample(sample, start.value_or(0u), end.value_or(sample->nLength), SampleChannelSelection::Both, SndFile()))
		{
			SetModified(sample, SampleHint(static_cast<SAMPLEINDEX>(m_index)).Data(), true);
			return true;
		}
	...
	bool StretchHQ(float stretchFactor, sol::optional<float> pitchFactor, sol::optional<SmpLength> start, sol::optional<SmpLength> end)
	{
		...
		TimeStretchPitchShift::Signalsmith instance(updateFunc, prepareSampleUndoFunc, SndFile(), static_cast<SAMPLEINDEX>(m_index), pitchFactor.value_or(0.0f), stretchFactor, start.value_or(0), end.value_or(sample->nLength), SampleChannelSelection::Both);
	...
	bool StretchLofi(float stretchFactor, sol::optional<float> pitchFactor, sol::optional<int> grainSize, sol::optional<SmpLength> start, sol::optional<SmpLength> end)
	{
		...
		const auto updateFunc = [](SmpLength, SmpLength) { return false; };
		const auto prepareSampleUndoFunc = [] { /* TODO */ };
		TimeStretchPitchShift::LoFi instance(updateFunc, prepareSampleUndoFunc, SndFile(), static_cast<SAMPLEINDEX>(m_index), pitchFactor.value_or(0.0f), stretchFactor, start.value_or(0), end.value_or(sample->nLength), SampleChannelSelection::Both, grainSize.value_or(1024));
		return instance.Process() == TimeStretchPitchShift::Result::OK;
	}
	...
	bool Load(const std::string &filename)
	{
		...
		auto sndFile = SndFile();
		bool result = sndFile->ReadSampleFromFile(static_cast<SAMPLEINDEX>(m_index), file, false, true);
		if(result)
		{
			auto sample = GetSample();
			SetModified(sample, SampleHint(static_cast<SAMPLEINDEX>(m_index)).Info().Data().Names(), true);
