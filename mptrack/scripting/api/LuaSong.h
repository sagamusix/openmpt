...
struct Song : public Obj
{
	...
	{
		auto sndFile = SndFile();
		sndFile->m_nSamplePreAmp = static_cast<uint32>(Clamp(volume, 0.0, 2000.0)); // TODO convert from dB or at least map 1.0 to 0dB?
		SetModified(sndFile, GeneralHint().General());
	}
	...
	{
		auto sndFile = SndFile();
		sndFile->m_nVSTiVolume = static_cast<uint32>(Clamp(volume, 0.0, 2000.0)); // TODO convert from dB or at least map 1.0 to 0dB?
		SetModified(sndFile, GeneralHint().General());
	}
	...
	{
		auto sndFile = SndFile();
		sndFile->m_nDefaultGlobalVolume = static_cast<uint32>(Clamp(volume, 0.0, 1.0) * 256.0);
		SetModified(sndFile, GeneralHint().General());
	}
	...
	{
		SndFile()->m_PlayState.m_nGlobalVolume = static_cast<uint32>(Clamp(volume, 0.0, 1.0) * 256.0);
	}
	...
	{
		return SndFile()->m_nDefaultTempo.ToDouble();
	}
	void SetTempo(double tempo)
	{
		...
		{
			sndFile->m_nDefaultTempo = DoubleToTempo(tempo);
			SetModified(sndFile, GeneralHint().General());
		}
	}
	...
	{
		return SndFile()->m_nDefaultSpeed;
	}
	void SetSpeed(uint32 speed)
	{
		...
		{
			const auto &specs = sndFile->GetModSpecifications();
			sndFile->m_nDefaultSpeed = Clamp(speed, specs.speedMin, specs.speedMax);
			SetModified(sndFile, GeneralHint().General());
		}
	}
