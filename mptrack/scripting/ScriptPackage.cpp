...
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE_WITH_DEFAULT(ScriptPackage::Manifest, author, description, version, api_version, permissions, entry_point)
	...
	: m_archiveName{filename.GetFilename()}
{
	...
	for(const auto &p : m_manifest.permissions)
	{
		requestedPermissions.insert(mpt::ToCharset(mpt::Charset::UTF8, p));
	}
...
sol::protected_function_result ScriptPackage::Run()
{
	...
	const std::string luaFilename = "@" + m_archiveName.ToUTF8() + "/" + filename.ToUTF8();  // Prefix with @ for proper stacktrace formatting
	const std::string moduleContent = mpt::buffer_cast<std::string>(m_archive->GetOutputFile().ReadRawDataAsByteVector());
	return lua.do_string(moduleContent, m_environment, luaFilename);
}
...
int ScriptPackage::SearchFile(lua_State *L)
{
	...
	const std::string filename = "@" + instance->m_archiveName.ToUTF8() + "/" + name;  // Prefix with @ for proper stacktrace formatting
	const std::string moduleContent = mpt::buffer_cast<std::string>(instance->m_archive->GetOutputFile().ReadRawDataAsByteVector());
