...
void Script::SetupOSLibrary()
{
	sol::table os = lua["os"];
	os["exit"].set_function([]() { throw "This function is not available."; });
	...
	{
		m_tempFiles.push_back(mpt::CreateTempFileName(MPT_PATHSTRING("openmpt_scripting")));
		return m_tempFiles.back().ToUTF8();
	});
#ifdef WIN32 // wchar fixups
...
Script::~Script()
{
	...
	{
		DeleteFileW(path.AsNativePrefixed().c_str());
	}
}
...
void Script::OnInit()
{
	...
	if(table.is<sol::table>())
	{
		...
		if(apiVersion > Scripting::API_VERSION)
			throw Error(MPT_FORMAT("This script expects API version {}. OpenMPT's API version is {}.")(apiVersion, Scripting::API_VERSION));
